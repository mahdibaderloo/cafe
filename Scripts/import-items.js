import { sadRobosta, vaftad30Robosta, vaftad30Arabica, sadArabica, hot, coldCoffee_100r, coldCoffee_7030r, coldCoffee_7030a, coldCoffee_100a, shake, cold, cake, snack, tea_1, tea_2, bubbleTea } from "../Scripts/db.js";
...
function createListItems (productTitle , productPrice, productDisc, productID) {
    ...
    let newItemLi = `<li class="item"><div class="item-top"><img class="item-img" src="Images/Cafe-Au-Lait-001s.jpg" alt=""><div class="item__content"><h3 class="item-title">${productTitle}</h3><p class="item-desc">${productDisc}</p></div></div><div class="item-bottom"><p class="item-price">${productPrice.toLocaleString('EN')}</p><button class="item-btn" onclick="updateProductsArray('${productTitle}' , ${productPrice}, ${productID})">افزودن به سفارشات</button></div></li>`
    ...
    items.forEach( item => {
            ...
            <div class="product-left-content">
                <div class="product-count">
                    <span class="product-count-plus" onclick="changePrice(this, ${item.id}, ${item.price})">+</span>
                    <span class="product-count-number">${item.count}</span>
                    <span class="product-count-minus" onclick="changePrice(this, ${item.id}, ${item.price})">-</span>
                </div>
                <h3 class="product-price">${item.price.toLocaleString('EN')}</h3>
            </div>
            </li>`
...
function changePrice (el, id, price) {
    ...
    let prePriceElement = el.parentElement.parentElement.querySelector('.product-price')
